...
src_prepare() {
	...
	prepare_funtoo_pip_pth() {
		sed "s/\$EPYTHON/$EPYTHON/g" "${FILESDIR}"/funtoo-pip.pth > "${WORKDIR}"/funtoo-pip-${EPYTHON}.pth
	}

	python_foreach_impl prepare_funtoo_pip_pth
}
...
python_install_all() {
	...
	change_pip_prefix() {
		insinto /etc
		newins "${FILESDIR}/pip.conf" pip.conf
		insinto $(python_get_sitedir $EPYTHON)
		newins "${WORKDIR}/funtoo-pip-$EPYTHON.pth" funtoo-pip.pth
	}

	python_foreach_impl change_pip_prefix

	newenvd "${FILESDIR}/env.d" 95pip
}
